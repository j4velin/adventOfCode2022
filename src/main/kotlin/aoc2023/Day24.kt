...
object Day24 {
    ...
    fun part1(input: List<String>, min: Long, max: Long): Int {
        ...
        val hailstones = input.map { Hailstone.fromString(it) }
        return hailstones.asSequence().withEachOf(hailstones.asSequence()).filter { it.first != it.second }.filter {
            val intersection = it.first.intersect(it.second)
            intersection != null
                    && intersection.first in range && intersection.second in range
                    && !it.first.inThePast(intersection) && !it.second.inThePast(intersection)
        }.count() / 2
    }
